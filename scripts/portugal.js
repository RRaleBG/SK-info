...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Portugal", code: "PT", items } : null;
}

export async function getPortugalBans() {
    const url = "https://truckban.eu/Portugal";
    try {
        ...
        const local = extractSection($, "p", t => /Lisbon|Porto|Coimbra|Braga/i.test(t), "Lokalne zabrane");
        ...
        console.error("getPortugalBans error:", e.message);
        return [];
    }
}
