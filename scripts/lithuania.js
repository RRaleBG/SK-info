...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Litvanija", code: "LT", items } : null;
}

export async function getLithuaniaBans() {
    const url = "https://truckban.eu/Lithuania";
    try {
        ...
        const local = extractSection($, "p", t => /Vilnius|Kaunas|Klaipeda|Siauliai/i.test(t), "Lokalne zabrane");
        ...
        console.error("getLithuaniaBans error:", e.message);
        return [];
    }
}
