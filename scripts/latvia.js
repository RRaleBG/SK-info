...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Letonija", code: "LV", items } : null;
}

export async function getLatviaBans() {
    const url = "https://truckban.eu/Latvia";
    try {
        ...
        const local = extractSection($, "p", t => /Riga|Daugavpils|Liepaja|Jelgava/i.test(t), "Lokalne zabrane");
        ...
        console.error("getLatviaBans error:", e.message);
        return [];
    }
}
