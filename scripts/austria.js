...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Austrija", code: "AT", items } : null;
}

export async function getAustriaBans() {
    const url = "https://truckban.eu/Austria";
    try {
        ...
        const local = extractSection($, "p", t => /Vienna|Tirol|Salzburg/i.test(t), "Lokalne zabrane");
        ...
        console.error("getAustriaBans error:", e.message);
        return [];
    }
}
