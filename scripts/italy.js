...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Italija", code: "IT", items } : null;
}

export async function getItalyBans() {
    const url = "https://truckban.eu/Italy";
    try {
        ...
        console.error("getItalyBans error:", e.message);
        return [];
    }
}
