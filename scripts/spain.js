...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Španija", code: "ES", items } : null;
}

export async function getSpainBans() {
    const url = "https://truckban.eu/Spain";
    try {
        ...
        const local = extractSection($, "p", t => /Madrid|Barcelona|Valencia|Seville/i.test(t), "Lokalne zabrane");
        ...
        console.error("getSpainBans error:", e.message);
        return [];
    }
}
