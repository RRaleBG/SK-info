...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Holandija", code: "NL", items } : null;
}

export async function getNetherlandsBans() {
    const url = "https://truckban.eu/Netherlands";
    try {
        ...
        const local = extractSection($, "p", t => /Amsterdam|Rotterdam|Utrecht|Eindhoven/i.test(t), "Lokalne zabrane");
        ...
        console.error("getNetherlandsBans error:", e.message);
        return [];
    }
}
