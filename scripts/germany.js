...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Nemačka", code: "DE", items } : null;
}

export async function getGermanyBans() {
    const url = "https://truckban.eu/Germany";
    try {
        ...
        const local = extractSection($, "p", t => /Berlin|Hamburg|Munich|Stuttgart/i.test(t), "Lokalne zabrane");
        ...
        console.error("getGermanyBans error:", e.message);
        return [];
    }
}
