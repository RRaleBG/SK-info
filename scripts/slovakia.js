...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Slovačka", code: "SK", items } : null;
}

export async function getSlovakiaBans() {
    const url = "https://truckban.eu/Slovakia";
    try {
        ...
        const local = extractSection($, "p", t => /Bratislava|Kosice|Zilina|Nitra/i.test(t), "Lokalne zabrane");
        ...
        console.error("getSlovakiaBans error:", e.message);
        return [];
    }
}
