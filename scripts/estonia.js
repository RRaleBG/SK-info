...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Estonija", code: "EE", items } : null;
}

export async function getEstoniaBans() {
    const url = "https://truckban.eu/Estonia";
    try {
        ...
        const local = extractSection($, "p", t => /Tallinn|Tartu|Narva|Parnu/i.test(t), "Lokalne zabrane");
        ...
        console.error("getEstoniaBans error:", e.message);
        return [];
    }
}
