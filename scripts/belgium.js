...
function extractSection($, selector, matchFn, sectionName) {
    ...
    return items.length ? { section: sectionName, country: "Belgija", code: "BE", items } : null;
}

export async function getBelgiumBans() {
    const url = "https://truckban.eu/Belgium";
    try {
        ...
        const local = extractSection($, "p", t => /Brussels|Antwerp|Ghent|Liege/i.test(t), "Lokalne zabrane");
        ...
        console.error("getBelgiumBans error:", e.message);
        return [];
    }
}
